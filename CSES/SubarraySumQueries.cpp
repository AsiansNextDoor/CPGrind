...
ll tot[800005][4]; // max subarray, max left prefix, max right prefix, all
...
void update(ll p, ll l, ll r, ll idx, ll val) {
    ...
    max({tot[p][1],tot[p][2],tot[p][3],tot[2 * p][2] + tot[2 * p + 1][1],tot[2 * p][0],tot[2 * p + 1][0]});
}
...
signed main() {
    ...
    while (m--) {
        ll x, y; cin >> x >> y;
        update(1, 1, n, x, y);
        cout << max(query(1, 1, n, 1, n)[0], 0LL) << endl;
    }
}